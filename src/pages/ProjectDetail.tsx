...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const [projRes, sovRes, cfRes, drawRes, docRes] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Project Header */}
      <div className="rounded-xl bg-[hsl(220,60%,18%)] text-white p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <Stat label="Avance" value={`${project.progress_pct ?? 0}%`} />
          <Stat label="Loan Amount" value={fmt(project.loan_amount)} />
          <Stat label="Ejecutado (EAC)" value={fmt(project.eac)} className={eacWarning ? 'text-orange-400' : ''} />
          <Stat label="CO Target" value={project.co_target_date ?? '—'} />
        </div>
            ...
            <Card className="border-0 shadow-sm">
              <CardContent className="p-5">
                ...
                <div className="space-y-3">
                  <Row label="Loan Amount" value={fmt(project.loan_amount)} />
                  <Row label="Ejecutado (EAC)" value={fmt(project.eac)} className={eacWarning ? 'text-orange-500 font-bold' : ''} />
                  <Row label="Disponible" value={fmt((project.loan_amount ?? 0) - (project.eac ?? 0))} />
                  <Row label="CO Target Date" value={project.co_target_date ?? '—'} />
                </div>
              </CardContent>
            </Card>
